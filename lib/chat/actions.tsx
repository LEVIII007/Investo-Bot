...
import { StockRecommendations } from '@/components/stocks/stock-reccomendation'
...
10.showStockRecommendations (based on user preferences, give stock recommendations)
...
Example:
User: Recommend stocks for me based on low risk and tech sector.
Assistant: { "tool_call": { "name": "showStockRecommendations", "parameters": { "preferences": "low risk, tech sector" } } }

Assistant (you): Here are some low-risk stocks in the tech sector: AAPL, MSFT, GOOG. I can provide more details if needed.
...
Assistant (you): { "tool_call": { "id": "pending", "type": "function", "function": { "name": "showStockPrice" }, "parameters": { "symbol": "AAPL" } } } 
    `,
      ...
      tools: {
        showStockRecommendations: {
          description:
            'Show stock recommendations based on user preferences (e.g., risk tolerance, sector preference).',
          parameters: z.object({
            preferences: z
              .string()
              .describe('User preferences for stock recommendations (e.g., low risk, tech sector).'),
          }),
          generate: async function* ({ preferences }) {
            yield (
              <BotCard>
                <></>
              </BotCard>
              ...
              messages: [
                ...aiState.get().messages,
                {
                  ...
                  content: [
                    {
                      type: "tool-call",
                      toolName: "showStockRecommendations",
                      toolCallId,
                      args: { preferences },
                    },
                  ],
            ...
            const recommendations = ["AAPL", "MSFT", "GOOG"]; // Replace this with actual AI result for recommendations
        
            const caption = `Based on your preferences (${preferences}), here are some stock recommendations: ${recommendations.join(
              ", "
            )}.`;
    ...
    // If key is missing, show error message that GPT API Key is missing.
    if (err.message.includes('OpenAI API key is missing.')) {
      err.message =
        'OpenAI API key is missing. Pass it using the OPENAI_API_KEY environment variable. Try restarting the application if you recently changed your environment variables.'
    }
    return {
      id: nanoid(),
      display: (
